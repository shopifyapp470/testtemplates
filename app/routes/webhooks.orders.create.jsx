...
export const action = async ({ request }) => {
  ...
  if (topic === "ORDERS_CREATE" || topic === "orders/create") {
    ...
    if (payload.discount_applications?.length > 0) {
      ...
      const employeeEmail = employeeAttr ? employeeAttr.value : null;
 
      if (employeeEmail) {
        // 1. Employee aur Customer email ka comparison (Self-referral check)
        const customerEmail = payload.email || payload.customer?.email;
 
        if (employeeEmail.toLowerCase() === customerEmail?.toLowerCase()) {
          console.log(`🚫 Skip: Self-referral detected for ${employeeEmail}. Tracking cancelled.`);
        } else {
          try {
            // 2. Idempotency Check: Kya ye orderId pehle se exist karti hai?
            ...
            if (!existingEntry) {
              ...
              await db.employeeReferral.create({
                data: {
                  ...
                  customerEmail: customerEmail ? customerEmail.trim() : "Guest",
                  shop: shop
                }
              });
              console.log(`✅ Success: Order ${payload.name} tracked for ${employeeEmail}`);
            } else {
              console.log(`⚠️ Skip: Duplicate referral entry for Order ${payload.name}`);
            }
          } catch (error) {
            console.error("❌ Database sync error in Employee Referral:", error.message);
          }
        }
      }
      /*** end employee share link ***/
    ...
    try {
      ...
      // --- B. INSTANT EARN LOGIC (For 10,000+ Orders) ---
        if (orderTotal >= THRESHOLD) {
            ...
            console.log(`🚀 [INSTANT CREDIT] High Value Order! Adding ${pointsEarnedForHistory} points.`);

            // ✅ DB Update: Direct increment in activepoint & pointvalue (No delay)
      ...
      if (discountUsed > 0 && admin && isAutoGeneratedReward) {
        try {
          ...
          if (targetDiscount) {
            ...
            console.log(`🗑️ [SHOPIFY] Found ${discountType}. Deleting ID: ${discountId}`);
            // ✅ FIXED MUTATION: Shopify mein delete karne ka sahi mutation
            ...
            if (deletedId) {
                ...
                console.log("⚠️ [SHOPIFY DELETE FAIL]", JSON.stringify(deleteData.data?.discountCodeDelete?.userErrors, null, 2));
            }
          } 
          // if (targetDiscount.length > 0) {
          //   console.log(`🗑️ Found ${targetDiscounts.length} matching discounts. Starting Bulk Delete...`);
          ...
          //           console.log(`✅ [DELETED] ${discountTitle}`);
          //       } else {
          //           console.log(`⚠️ [FAILED] ${discountTitle}:`, JSON.stringify(deleteData.data?.discountCodeDelete?.userErrors));
          //       }
          //   });
          ...
          // }
              ...
              console.log("ℹ️ [SHOPIFY] No matching active discount found to delete.");
          }
        } catch (e) { 
    ...
    try {
     const orderIdToFind = payload.admin_graphql_api_id || `gid://shopify/Order/${payload.id}`;
        ...
        if (userReward) {
            ...
            // ✅ FIX: pointsRevert mein hum wahi value dalenge jo revert ho rahi hai
...
// ----------------------------------------------------
  ...
  //     console.log(`🔍 [CHECKOUT] Reward discount removed/not found for Customer: ${customerIdString}`);
  ...
  //       if (targetDiscount) {
  //         const discountId = targetDiscount.node.id;
  //         console.log(`🗑️ [DELETE] Removing Price Rule ID: ${discountId} from Shopify Admin`);
  ...
  //         console.log("✅ [SUCCESS] Abandoned discount deleted.");
  //       }
  //     } catch (err) {
